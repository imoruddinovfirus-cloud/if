...
def check_payment():
    ...
    external_id = request.args.get('externalId', 'unknown')
    
    # Простое логирование
    logger.info(f"✅ /check_payment запрос: externalId={external_id}")
    logger.info(f"✅ Возвращаем успешный ответ для любого запроса")
    
    # ВСЕГДА ВОЗВРАЩАЕМ УСПЕХ
    return jsonify({
        "success": True,
        "message": f"✅ Оплата подтверждена! (externalId: {external_id})",
        "status": "confirmed",
